...
struct _Has_GetMiscStatus : ::std::false_type{};
template <class T>
struct _Has_GetMiscStatus<T, ::std::enable_if_t<::std::is_convertible_v<decltype(T::_GetMiscStatus()), int>>> : ::std::true_type{};
...
template <class T, class = void>
struct _Has_InternalFinalConstructAddRef : ::std::false_type{};
template <class T>
struct _Has_InternalFinalConstructAddRef<T, ::std::void_t<decltype(::std::declval<T>().InternalFinalConstructAddRef())>> : ::std::true_type{};

template <class T, class = void>
struct _Has_InternalFinalConstructRelease : ::std::false_type{};
template <class T>
struct _Has_InternalFinalConstructRelease<T, ::std::void_t<decltype(::std::declval<T>().InternalFinalConstructRelease())>> : ::std::true_type{};

template <class T, class = void>
struct _Has_FinalRelease : ::std::false_type{};
template <class T>
struct _Has_FinalRelease<T, ::std::void_t<decltype(::std::declval<T>().FinalRelease())>> : ::std::true_type{};

template <class T, class = void>
struct _Has_GetAttrEntries : ::std::false_type{};
template <class T>
struct _Has_GetAttrEntries<T, ::std::void_t<::std::enable_if_t<::std::is_same_v<const ATL::_ATL_INTMAP_ENTRY*, decltype(T::_GetAttrEntries())>>>> : ::std::true_type{};
...
#ifdef _ATL_MODULES
#define BEGIN_PROPERTY_MAP_WITHOUT_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
	BEGIN_PROP_MAP_WITHOUT_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
	PROP_DATA_ENTRY("_cx", m_sizeExtent.cx, VT_UI4) \
	PROP_DATA_ENTRY("_cy", m_sizeExtent.cy, VT_UI4)

#define BEGIN_PROPERTY_MAP_WITH_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
	BEGIN_PROP_MAP_WITH_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
	PROP_DATA_ENTRY("_cx", m_sizeExtent.cx, VT_UI4) \
	PROP_DATA_ENTRY("_cy", m_sizeExtent.cy, VT_UI4)
...
// Note that these below macros do NOT exactly emulate the behavior of BEGIN_PROP_MAP (using __if_not_exists), 
// 		as there is no way to conditionally define a typedef outside of a method, but inside of a class 
// 		(especially when we don't know anything about the class in which this macro is being used). Instead, 
//		we define new macros that can be used to emulate the behavior, depending on whether the user wants to 
//		use their own event class and typedef it as __ATL_PROP_NOTIFY_EVENT_CLASS (with typedef version) or
//		they can use ours (withOUT typedef version). 
#define BEGIN_PROP_MAP_WITHOUT_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
...
#define BEGIN_PROP_MAP_WITH_ATL_PROP_NOTIFY_EVENT_CLASS_TYPEDEF(theClass) \
	typedef theClass _PropMapClass; \
	static const ATL::ATL_PROPMAP_ENTRY* GetPropertyMap()\
	{\
		static const ATL::ATL_PROPMAP_ENTRY pPropMap[] = \
...
#define BEGIN_COM_MAP(x) public: \
	...
	template <class _Ty = _ComMapClass> \
    static constexpr ATL::_ATL_INTMAP_ENTRY _Get_entries_helper() { \
        if constexpr (ATL::_Has_GetAttrEntries<_Ty>::value) { \
            return {NULL, (DWORD_PTR)_Ty::_GetAttrEntries, _Ty::_ChainAttr}; \
        } else { \
            return {NULL, 0, 0}; \
        } \
    } \
...
#define DECLARE_GET_CONTROLLING_UNKNOWN() public:\
	virtual IUnknown* GetControllingUnknown() throw() {return this->GetUnknown();}
...
// DECLARE_REGISTRY_RESOURCE_V2 replicates the behavior of DECLARE_REGISTRY_RESOURCE but with an additional parameter for 
//		the name of the class in which the macro is used. Given the classname, the macro can determine whether the 
//		_GetMiscStatus() member and _Module exist. 
// Note that the template type parameter and _Ty::_GetMiscStatus() are required to ensure
// 		that the calls to functions are considered "templated entities" and are correctly discarded 
// 		when the if constexpr condition evaluates to false (meaning that those functions aren't 
// 		defined / available).
...
#define DECLARE_REGISTRY_RESOURCE_V2_WITH_MODULE(x, className)\
	template <class _Ty = className> \
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister)\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
		{ \
			...
			TCHAR szOleMisc[32]; \
			ATL::Checked::itot_s(_Ty::_GetMiscStatus(), szOleMisc, _countof(szOleMisc), 10); \
			USES_CONVERSION_EX; \
...
#define DECLARE_REGISTRY_RESOURCE_V2_WITHOUT_MODULE(x, className)\
	template <class _Ty = className> \
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister)\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
...
#ifdef _ATL_MODULES
// Note that the following implementation of DECLARE_REGISTRY_RESOURCE removes the uses of __if_exists, but in a non-Standard 
...
//		class. Whenever possible, users instead should use DECLARE_REGISTRY_RESOURCE_V2 above which takes the class name 
//		as a parameter rather than using this non-Standard extension to determine this information. 
// Similarly, we cannot detect whether the global variable _Module has been declared or not in a Standard way (without the
//		use of __if_(not_)?exists), so we declare two versions of each macro: one where a _Module global variable has been
//		declared by the user that we use (with module version), and one where we use ATL::_pAtlModule instead (without 
//		module version). 
#define DECLARE_REGISTRY_RESOURCE_WITH_MODULE(x)\
	template <class _Pmf> \
	struct _GetClassType;\
	template <class _Class> \
	struct _GetClassType<void (_Class::*)()> {\
		using _Type = _Class;\
	};\
	void _DummyNonStaticMemberFunction();\
	using _Self = typename _GetClassType<decltype(&_DummyNonStaticMemberFunction)>::_Type;\
	template <class _Ty = _Self>\
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister)\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
		{ \
...
#define DECLARE_REGISTRY_RESOURCE_WITHOUT_MODULE(x)\
	template <class _Pmf> \
	struct _GetClassType;\
	template <class _Class> \
	struct _GetClassType<void (_Class::*)()> {\
...
// DECLARE_REGISTRY_RESOURCEID_V2 replicates the behavior of DECLARE_REGISTRY_RESOURCEID but with an additional parameter for 
//		the name of the class in which the macro is used. Given the classname, the macro can determine whether the 
//		_GetMiscStatus() member and _Module exist. 
// Note that the template type parameter and _Ty::_GetMiscStatus() are required to ensure
// 		that the calls to functions are considered "templated entities" and are correctly discarded 
...
//		use of __if_(not_)?exists), so we declare two versions of each macro: one where a _Module global variable has been
//		declared by the user that we use, and one where we use ATL::_pAtlModule instead. 
#define DECLARE_REGISTRY_RESOURCEID_V2_WITH_MODULE(x, className)\
	template <class _Ty = className> \
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister) throw()\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
...
#define DECLARE_REGISTRY_RESOURCEID_V2_WITHOUT_MODULE(x, className)\
	template <class _Ty = className> \
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister) throw()\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
...
#define DECLARE_REGISTRY_RESOURCEID_WITH_MODULE(x)\
	template <class _Pmf> \
	struct _GetClassType_ID;\
	template <class _Class> \
	struct _GetClassType_ID<void (_Class::*)()> {\
		using _Type = _Class;\
	};\
	void _DummyNonStaticMemberFunction_ID();\
	using _Self_ID = typename _GetClassType_ID<decltype(&_DummyNonStaticMemberFunction_ID)>::_Type;\
	template <class _Ty = _Self_ID>\
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL bRegister) throw()\
	{\
		if constexpr (ATL::_Has_GetMiscStatus<_Ty>::value) \
		{ \
...
public:
...
#ifdef _ATL_MODULES
		// Note that the template type parameter and static_cast<T&>(*this) are required to ensure
		// that the calls to functions are considered "templated entities" and are correctly discarded 
		// when the if constexpr condition evaluates to false (meaning that those functions aren't 
		// defined / available).
	...
	// when the if constexpr condition evaluates to false (meaning that those functions aren't 
	// defined / available).
	// We cannot templatize a destructor, so we create _Workaround_if_exists as a helper function
	// to conditionally call FinalRelease() if the function exists. 
	template <class _Ty = CComObjectGlobal>
	void _Workaround_if_exists() {
		if constexpr (_Has_FinalRelease<_Ty>::value)
		{
			static_cast<_Ty&>(*this).FinalRelease();
		}
	}
#endif // _ATL_MODULES
...
// Note: the explicit checking and nullifying of *ppvObject is a temporary workaround for OS Bug: 38109112
#define SERVICE_ENTRY_CHAIN(x) \
		ATL::CComQIPtr<IServiceProvider, &__uuidof(IServiceProvider)> spProvider(x); \
		if (spProvider != NULL) \
		{ \
            auto hr = spProvider->QueryService(guidService, riid, ppvObject); \
            if (FAILED(hr)) *ppvObject = nullptr; \
			return hr; \
		}
...
template <class T, class = void>
struct _Has_GetAttrConnMap : ::std::false_type{};
template <class T>
struct _Has_GetAttrConnMap<T, ::std::void_t<::std::enable_if_t<::std::is_same_v<const ATL::_ATL_CONNMAP_ENTRY*, decltype(T::GetAttrConnMap(::std::declval<int*>()))>>>> : ::std::true_type{};

// We want the offset of the connection point relative to the connection
// point container base class
#ifdef _ATL_MODULES
// Note that these below macros do NOT exactly emulate the behavior of BEGIN_CONNECTION_POINT_MAP and 
//		BEGIN_ATTRCONNECTION_POINT_MAP (using __if_not_exists), as there is no way to conditionally define 
// 		a typedef outside of a method, but inside of a class (especially when we don't know anything about 
// 		the class in which this macro is being used). Instead, we define new macros that can be used to 
//		emulate the behavior, depending on whether the user wants to use their own connection class type
//		and typedef it as _atl_conn_classtype (without typedef version) or they can use ours (with typedef 
//		version). 
#define BEGIN_CONNECTION_POINT_MAP_WITHOUT_ATL_CONN_CLASSTYPE_TYPEDEF(x)\
	typedef x _atl_conn_classtype;\
	typedef x _Self;\
	static constexpr ATL::_ATL_CONNMAP_ENTRY _Get_entries_helper1() { \
		if constexpr (ATL::_Has_GetAttrConnMap<x>::value) { \
			return {(DWORD_PTR) -2}; \
		} else { \
			return {(DWORD_PTR) -1}; \
		} \
	} \
	template <class _Ty = x> \
	static constexpr ATL::_ATL_CONNMAP_ENTRY _Get_entries_helper2() { \
		if constexpr (ATL::_Has_GetAttrConnMap<_Ty>::value) { \
			return {(DWORD_PTR) _Ty::GetAttrConnMap}; \
		} else { \
			return {(DWORD_PTR) -1}; \
		} \
	} \
...
#define BEGIN_CONNECTION_POINT_MAP_WITH_ATL_CONN_CLASSTYPE_TYPEDEF(x)\
	typedef x _Self;\
	static constexpr ATL::_ATL_CONNMAP_ENTRY _Get_entries_helper1() { \
		if constexpr (ATL::_Has_GetAttrConnMap<x>::value) { \
			return {(DWORD_PTR) -2}; \
...
#define BEGIN_ATTRCONNECTION_POINT_MAP_WITH_ATL_CONN_CLASSTYPE_TYPEDEF(x)\
	typedef x _Self;\
	static const ATL::_ATL_CONNMAP_ENTRY* GetAttrConnMap(_Out_opt_ int* pnEntries) {\
	static const ATL::_ATL_CONNMAP_ENTRY _entries[] = {

#define BEGIN_ATTRCONNECTION_POINT_MAP_WITHOUT_ATL_CONN_CLASSTYPE_TYPEDEF(x)\
	typedef x _atl_conn_classtype;\
	typedef x _Self;\
	static const ATL::_ATL_CONNMAP_ENTRY* GetAttrConnMap(_Out_opt_ int* pnEntries) {\
	static const ATL::_ATL_CONNMAP_ENTRY _entries[] = {
...
#define END_CONNECTION_POINT_MAP() \
	_Get_entries_helper1(), \
	_Get_entries_helper2(), \
	{(DWORD_PTR)-1} }; \
	if (pnEntries) \
	{ \
		if constexpr (ATL::_Has_GetAttrConnMap<_Ty>::value) \
		{ \
			_Ty::GetAttrConnMap(pnEntries); \
		...
		_Outptr_ IEnumConnectionPoints** ppEnum)
	{
		...
		*ppEnum = NULL;
		...
		while (pEntry->dwOffset != (DWORD_PTR)-1)
		{
			...
			__pragma(warning(suppress:6386)) // VSO-1804675
